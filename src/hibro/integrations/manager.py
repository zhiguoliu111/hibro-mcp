...
Dynamically loads, manages, and coordinates integrations for different IDEs
"""
...
class IDEManager:
    ...
    def _load_integrations(self):
        ...
        for module_name in integration_modules:
            try:
                ...
                # Get integration class (convention: {ModuleName}Integration)
                class_name = f'{module_name.replace("_", "").title()}Integration'
    ...
    def get_active_integration(self) -> Optional[IDEIntegration]:
        """Get currently active IDE integration"""
    ...
    def start_monitoring(self, callback):
        ...
        if active_integration:
            logger.info(f"Started monitoring conversation files for {active_integration.get_name()}")
    ...
    def inject_context_to_active_ide(self, context: str, target_path: Optional[Path] = None) -> bool:
        """Inject context to currently active IDE"""
        active_integration = self.get_active_integration()
        if active_integration:
    ...
    def get_project_context_from_active_ide(self, project_path: Path) -> Optional[Dict]:
        """Get project context from currently active IDE"""
        active_integration = self.get_active_integration()
        if active_integration:
